...
const DISCOUNT = 0.7; // 30% off = pay 70%

const formatCLP = (n: number) => n.toLocaleString("es-CL");
const discounted = (n: number) => Math.round(n * DISCOUNT);
...
export default function PricingEstudiantes() {
  const ref = useInView();
    ...
    <section id="precio" className="py-16 sm:py-24 lg:py-32 relative overflow-hidden" ref={ref}>
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-10 sm:mb-14">
          <span className="fade-up inline-flex items-center gap-2 bg-violet-500/10 border border-violet-500/20 text-violet-600 px-4 py-1.5 rounded-full text-sm font-medium">
            🎓 Planes con 30% de descuento por 6 meses
          </span>
          <h2 className="fade-up fade-up-delay-1 mt-3 sm:mt-4 text-2xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight">
            Tu web dental{" "}
            <span className="gradient-text">con precio de estudiante</span>
          </h2>
        ...
        <div className="grid lg:grid-cols-3 gap-5 lg:gap-6 max-w-[420px] lg:max-w-none mx-auto">
          {plans.map((plan, i) => {
            ...
            const price = discounted(originalPrice);
            const originalAnnualSaving = plan.annualSaving;
            const intakeUrl = `/estudiantes/intake?plan=${plan.name.toLowerCase()}`;
            ...
            return (
              ...
              >
                ...
                <div className="p-6 sm:p-7 flex flex-col flex-1">
                  {/* Student badge */}
                  <div className="mb-3">
                    <span className="inline-flex items-center gap-1 bg-violet-500/10 text-violet-600 text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full">
                      🎓 -30% por 6 meses
                    </span>
                  </div>
                    ...
                    <div className="h-5 mt-1">
                      {isAnnual && (
                        <p className="text-amber-600 text-xs font-medium">
                          Ahorras ${formatCLP(discounted(originalAnnualSaving))} al año
                        </p>
                      )}
                    </div>
                  </div>
        ...
        <div className="fade-up fade-up-delay-4 mt-12 sm:mt-14">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 max-w-3xl mx-auto">
            ...
            {/* Dcto estudiante — highlighted */}
            <div className="flex flex-col items-center text-center bg-bg-card border-2 border-violet-500/30 rounded-xl p-4 sm:p-5 shadow-sm shadow-violet-500/5">
              <div className="w-10 h-10 bg-violet-500 rounded-full flex items-center justify-center text-white mb-2">
                <span className="text-lg">🎓</span>
              </div>
              <span className="text-sm font-bold text-text-primary">Dcto Estudiante</span>
              <span className="text-[11px] text-text-muted mt-0.5">30% OFF por 6 meses</span>
            </div>
          </div>
